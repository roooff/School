...
namespace task_for_matura
{
    public partial class FormPointAndRectangle : Form
    {
        ...
        private void Draw()
        {
            ...
            {
                //Начертава фона(whitearea)
                g.Clear(Color.White);
                //Начертава правоъгълника
        ...
        {
            return Math.Max(val1, Math.Min(val2, val3));
        }
        //Показва местоположениетo на точката:Inside/Border/Outside
        private void DisplayPointLocation(decimal x1, decimal y1, decimal x2, decimal y2, decimal x, decimal y)
        {
            var left = Math.Min(x1, x2);
            var right = Math.Min(x1, x2);
            var top = Math.Min(y1, y2);
            var bottom = Math.Min(y1, y2);

            if (x > left && x < right){
                this.labelLocation.Text = "Inside";
                this.labelLocation.BackColor = Color.PaleGreen;
            }
